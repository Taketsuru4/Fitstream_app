-- FitStream Booking System Database Schema - FIXED VERSION
...
-- 4. Create indexes for better performance (before adding foreign keys)
...
-- 5. Add foreign key constraint for booking_status_history
DO $$ 
BEGIN
  -- Add foreign key constraint if it doesn't exist
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'fk_booking_status_history_booking_id' 
...
DROP POLICY IF EXISTS "Trainers can manage their own availability" ON availability_slots;
...
DROP POLICY IF EXISTS "Clients and trainers can update their bookings" ON bookings;
DROP POLICY IF EXISTS "Users can view booking history for their bookings" ON booking_status_history;
...
-- Success message
DO $$ BEGIN
  RAISE NOTICE 'FitStream Booking System schema created successfully!';
  RAISE NOTICE 'Tables created: availability_slots, bookings, booking_status_history';
